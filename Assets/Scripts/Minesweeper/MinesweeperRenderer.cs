...
namespace Minesweeper
{
    public class MinesweeperRenderer : MonoBehaviour
    {
        ...
        private MinesweeperBoardManager _boardManager;
        ...
        public void DrawBoard(BoardData boardData)
        {
            ...
            for (int y = 0; y < boardData.BoardHeight; y++)
            {
                ...
                {
                    var cell = Instantiate(cellPrefab); //UnityEngine.Object.Instantiate(cellPrefab);
                    ...
                    if (cell.TryGetComponent<BoardCellController>(out var c))
                    {
                        ...
                        c.Init(x,y);
                        // c.OnClickCell += _boardManager.OnClickCell;
                        // c.OnClickCell += OnClickCell;
                        // c.OnClickCell += OnSelectCell;
                    }
        ...
        private void SetupVisualNode(MinesweeperCell cell, int x, int y)
        {
            if (cell.IsRevealed || revealEntireMap)
            {
                ...
                // _texts[y, x].gameObject.SetActive(cell.NearbyMineCount > 0);
                // _icons[y, x].gameObject.SetActive(cell.IsMine && !cell.IsFlagged);
            ...
            {
                // _hidden[y, x].gameObject.SetActive(!cell.IsFlagged);
                // _texts[y, x].gameObject.SetActive(false);
                // _icons[y, x].gameObject.SetActive(cell.IsFlagged);
